...
export default function EditorScreen() {
  ...
  const handleKeyPress = (key: string) => {
    ...
    switch (key) {
      case 'BACKSPACE':
        if (selection.start !== selection.end) {
          ...
          newContent = content.slice(0, selection.start) + content.slice(selection.end);
          newCursorPosition = selection.start;
        } else if (cursorPosition > 0) {
          // Sinon, supprimer le caractère avant le curseur
          newContent = content.slice(0, cursorPosition - 1) + content.slice(cursorPosition);
      ...
      default:
        ...
        if (selection.start !== selection.end) {
          ...
          newContent = content.slice(0, selection.start) + key + content.slice(selection.end);
          newCursorPosition = selection.start + key.length;
        } else {
          // Insérer à la position du curseur
          newContent = content.slice(0, cursorPosition) + key + content.slice(cursorPosition);
          newCursorPosition = cursorPosition + key.length;
        }
    }
    ...
    requestAnimationFrame(() => {
      ...
      // Mettre à jour la position du curseur avec un petit délai
      setTimeout(() => {
  ...
  const handleCursorPositionChange = (position: number, sel: { start: number, end: number }) => {
    // Éviter les mises à jour inutiles
    if (position !== cursorPosition || sel.start !== selection.start || sel.end !== selection.end) {
